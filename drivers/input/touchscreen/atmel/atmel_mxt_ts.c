...
extern int ltr559_get_ps_value_for_double_tap(void);
#endif
...
static ssize_t mxt_plugin_gesture_trace_show(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t plugin_functon_op_d_click(struct plug_interface *pl,char *buf, size_t count,
	unsigned long op, bool show);
#endif
...
static void update_keymap(struct mxt_platform_data *pdata)
{
	...
	pdata->keymap[T15_T97_KEY][2] = KEY_APP_SWITCH;
#endif
	return;
}
#endif
...
static ssize_t fts_gesture_proc_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	page = kzalloc(50, GFP_KERNEL);
		cnt = sprintf(page,  "mxt gesture open\n");
...
static ssize_t fts_gesture_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	char i;
	int ret;
	struct mxt_data *data = dev_get_drvdata(atmel_dev);
...
//	if (i == '1') {
		ret = plugin_functon_op_d_click(pl, &i, size, PL_FUNCTION_FLAG_WAKEUP_GESTURE, false);
		if (pl->active_thread)
			pl->active_thread(pl->dev,MXT_EVENT_EXTERN);
//	} else if(i == '0') {
//		ret = plugin_functon_op_d_click(pl, &i, size, PL_FUNCTION_FLAG_WAKEUP_GESTURE, false);
//	} else if(i=='2') {
//		mxt_suspend(atmel_dev);
//	}
